...
public class UserInterfaceInfoServiceImpl extends ServiceImpl<UserInterfaceInfoMapper, UserInterfaceInfo>
    ...
    public void validInterfaceInfo(UserInterfaceInfo userInterfaceInfo, boolean add) {
        ...
        if (add) {
            ...
            if (interfaceInfoId == null || interfaceInfoId <= 0) throw new BusinessException(ErrorCode.PARAMS_ERROR, "接口id不合法");
            if (totalNum == null || totalNum < 0) throw new BusinessException(ErrorCode.PARAMS_ERROR, "总次数不合法");
            if (leftNum == null || leftNum < 0 || leftNum > totalNum) throw new BusinessException(ErrorCode.PARAMS_ERROR, "剩余次数不合法");
        }
